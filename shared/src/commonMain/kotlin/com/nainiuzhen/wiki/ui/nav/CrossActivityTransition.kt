...
//
// 移植自 miuix example CrossActivityTransition.kt (v1.0.9/1206)。
// 原文件关键行为摘要：
// - 程序化 push/pop = 经典 Activity 窗口动画（两层各滑 96dp、固定 450ms
//   `fast_out_extra_slow_in` 两段三次贝塞尔时间线，顶层自带 83ms 墙钟 alpha 轨道，
//   无压暗、无缩放）。
// - 手势（预测性返回/边缘侧滑）= AOSP WM Shell 卡片动画：卡片以 0.9 最小缩放居中收拢、
//   竖向跟随手指、提交后沿 450ms 缓动轨道放大飞出并叠加闭式弹簧 fling 回弹
//   （stiffness 200 / damping 0.75，边缘滑动 kick 翻倍，clamp [120, 1000]）、
//   取消走 stiffness 1500 的硬弹簧。
// - 与 NavDisplayEffects(cornerClipMode = All, cornerClipRadius = 系统圆角) 搭配
//   即可完整复刻平台四角圆角卡片效果。
