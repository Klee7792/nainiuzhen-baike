...
// 移植自 miuix 示例 component.liquid.Lens（iOS 液态玻璃折射效果）。
// 注意：`lens()` 依赖的 `BackdropEffectScope.runtimeShaderEffect` 是 miuix-blur 的 **public** API，
// `:shared` 通过 includeBuild 引入 miuix 可直接访问；此前"shared 不可用"为误判。
