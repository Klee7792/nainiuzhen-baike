...
//
// 从 miuix 示例 `component.effect.OS3BgFrag` 移植（phone 配置）。
// 用于「关于」页的 OS3 风格动态背景（与 demo3 "Effect Variant=OS3" 一致）。

package com.nainiuzhen.wiki.ui.settings.about

/**
 * OS3 动态背景片段着色器（runtime shader）。
 * 输入：4 个带动画位置 + 半径的点，颜色在 4 组调色板之间循环插值，
 * 再叠加 Perlin 噪声 → 产生"光斑 + 渐变 + 颗粒"的 OS3 风格动态背景。
 */
internal const val OS3_BG_FRAG: String = """
